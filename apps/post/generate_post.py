...
class CaptionGeneratorState(dict):
    ...
    word_count: int
    within_limit: bool
    image_path: str  # NEW: input image path
...
# -----------------------------
def compile_social_caption_generator():
    llm = ChatOpenAI(model="gpt-4o-mini", temperature=0.8, max_tokens=600)

    # -----------------------------
    # IMAGE DESCRIPTION
    # -----------------------------
    def describe_image_node(state: CaptionGeneratorState):
        image_path = state.get("image_path")
        if not image_path:
            state["image_description"] = state.get("input_text", "")
            return state
        ...
        try:
            ...
            response = openai.chat.completions.create(
                ...
                messages=[
                    {"role": "user", "content": [
                        {"type": "input_image", "image_data": base64.b64encode(image_bytes).decode()},
                        {"type": "input_text", "text": "Describe this image in detail for social media caption creation."}
                    ]}
                ]
            )
            state["image_description"] = response.choices[0].message.content
        except Exception as e:
    ...
    def content_moderation(state: CaptionGeneratorState):
        text = (state.get("input_text") or "").lower()
        banned = ["hate", "violence", "porn", "nudity", "racist", "sexist", "harassment"]
...
"""
    ...
    graph.add_edge("describe_image", "moderation")
    graph.add_conditional_edges("moderation", route_after_moderation, {"rules": "rules", END: END})
    graph.add_edge("rules", "generate")
    graph.add_edge("generate", "format")
    graph.add_edge("format", END)
...
# PUBLIC HELPER FUNCTION
# -----------------------------
def generate_social_caption_from_image(
    image_path: str,
    platform: str = "instagram",
    tone: str = "friendly",
    topic: str = "",
...
if __name__ == "__main__":
    image_path = "my_post_image.jpg"
    output = generate_social_caption_from_image(
        image_path=image_path,
        platform="instagram",
        tone="inspirational",
        topic="travel",
